...
**`RegisterClassExW` 对已注册的类返回失败。** 类名已被占用时它返回 0，最后一次错误为
`ERROR_CLASS_ALREADY_EXISTS`（1410）。把任何返回 0 都当作错误上报，会让第二次注册失败。
密钥面板的类在 `createKeyPanel` 中注册，窗口启动时又注册了一次，于是面板在创建自己的
窗口之前就退出，症状仅为「点了 SSH keys 没有任何反应」——失败路径是在一个从未存在的
窗口上弹消息框。**注册一个已存在的类不是失败，是空操作。**
